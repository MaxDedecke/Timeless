...
*Zweite Revision (ebenfalls 24.08.2026, vollständiger Neudurchlauf):* Der Lauf wurde gegen einen frisch gestarteten Stack wiederholt und das Protokoll dabei an den tatsächlich beobachteten Stand angeglichen. Berichtigt wurden: (a) Die beiden Bestandsbuchungen liegen als **UTC**-Zeiten (Brainstorm-Bunker 15:00–16:00 UTC, Konferenz Nordsee 17:00–18:00 UTC). Ein Vorab-Probe der Suche 17:00–18:00 gegen zunächst falsch gesetzte Seed-Zeiten zeigte vier statt drei Treffer und löste die Zeitschicht-Prüfung aus – die Anwendung selbst blieb auch dabei fehlerfrei; Frontend und Backend interpretieren Zeiten durchgängig als UTC (Suche und Buchungsdialog senden „…T…:00Z", der Server prüft halboffene Intervalle in `timestamptz`, das Zeitgitter liest die Uhrzeit direkt aus dem ISO-String der API). Erst nach Korrektur des Datenstands lief der unten dokumentierte Durchlauf. (b) Nach Durchlauf 2 sind es **zwei** Räume mit insgesamt vier Belegungen (nicht „drei Räume"); außerdem zählt der Standort „DesignFreak HQ" vier Räume, nicht drei. Alle unten genannten Uhrzeiten sind daher zugleich Datenbank-UTC und UI-Anzeige.

## Vorbereitung

Der Stack lief bei `check_in_browser` ohne `seed-showcase.sh`, die Datenbank war leer (per `SELECT` verifiziert, bevor die Ansichten geprüft wurden). Stammdaten daher vorab per SQL idempotent angelegt (Name-JOIN, `WHERE NOT EXISTS`): Standorte „DesignFreak HQ" und „Studio Altona"; Räume Brainstorm-Bunker (6, Beamer+Whiteboard), Focus-Oase (4, Whiteboard), Konferenz Nordsee (12, Beamer+Videokonferenz+Whiteboard), Fotostudio Ost (20, alle drei); dazu zwei Bestandsbuchungen am Testtag 24.08.2026 als UTC-Zeiten (Brainstorm-Bunker 15:00–16:00 UTC, Konferenz Nordsee 17:00–18:00 UTC, Urheber „Facility-Team DesignFreak", Status `bestaetigt`).
...
5. `/day/1`: Alle vier HQ-Räume gegenübergestellt mit derselben Fensterfolge wie im Kalender-Einzelbild; jede Buchung am richtigen Raum im Raster; Studio Altona erscheint nicht als Spalte, sondern als Standortwechsel-Link (Anforderung 2). Hinweisband-Muster für freie Fenster.
...
4. `/day/1`: **zwei** Räume mit insgesamt **vier** Belegungen (Brainstorm-Bunker 15–16 und 17–18, Konferenz Nordsee 15–16 und 17–18), allesamt als „belegt" + Badge „Bestätigt"; keine Abweichung zum Kalender-Einzelbild.
...
- **Statusanzeige:** Grep nach Statusnutzung außerhalb `BookingStatusBadge.tsx`: die Treffer sind der HTTP-Status im Fetch-Helper (`pages/RoomList.tsx`: ``throw new Error(`HTTP ${res.status}`)``), der HTTP-Status eines Fehlers im Serienpfad des Raumkalenders (`reason.status === 404`), die Abfragestatus von `Promise.allSettled` in Raumkalender und Tagesansicht (`ergebnis.status === "rejected"` – Promise-Auflösung, kein Buchungsstatus) sowie die Badge-Weitergabe und Check-in-Fensternutzung in `components/TimeGrid.tsx` – keine Ansicht rendert einen rohen Buchungsstatuswert; Raumkalender und Tagesansicht zeigen den Badge ausschließlich über die gemeinsame Komponente (Zuordnung laut Konzeptstabelle, „Bestätigt" = `success` auch im Raster sichtbar).
...
Der Neudurchlauf der zweiten Revision bestätigte dieselbe Zwölf-Aufrufe-Sequenz mit identischen Ergebnissen (Treffermengen 3 → 2 → 3 → 2 → Leerzustand, Mobil 2); die abschließende SQL-Gegenprüfung umfasste alle **sechs** Buchungszeilen – zwei Bestandsbuchungen (15–16 bzw. 17–18 UTC, Urheber „Facility-Team DesignFreak") und die vier Dialog-Buchungen #3–#6 mit exakt den vorbelegten Suchzeiten und den Formular-Urhebern `frida.lang@`, `lena.meyer@`, `tim.becker@`, `sara.klein@designfreak.de`.
